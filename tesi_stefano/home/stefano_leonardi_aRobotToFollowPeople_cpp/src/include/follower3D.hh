 ...
 *  @author Stefano Leonardi and Paolo Bevilacqua
 ***********************************************/
#pragma once
...
enum ClassifierNames{RESNET50=0, GOOGLENET=1, GOOGLENET_TRT=2, OSNET=3};


/*! Follower3D is the main class of the project. It is a fusion of all the other classes and methods in order to effectively track in real-time a person in the real environment. */
class Follower3D{
    private:
        // Core variables
        ...
        unique_ptr<KNN> knn;
        unique_ptr<LoadVideo3D> videoIn;
        bool useCuda;          
        string negativePeopleDataset;

        // Main flow variables
        Rect2d defaultPosition;   //the value returned when the leader is not detected
        Rect2d breakPosition;     //the value returned to stop the execution of follow
        ...
        jm::circular_buffer<std::pair<float, float>, 100> hist;
        ...
        */
        void slowStartPhase(FrameData3D & frame, Rect2d & leaderPosition);
         ...
         *  \param[out] leaderPosition The computed location of the leader will be stored here. If the location fails a default position is used: (-1, -1). If the user ask to stop the processing a break position is returned: (-2, -2).
        */
        void leaderTrackingPhase(FrameData3D & frame, Rect2d & leaderPosition, bool est_pose = false, double est_x_h = 0, double est_y_h = 0);
        ...
        */
        ...
        bool bbox2World(const FrameData3D & frame, const Rect2d & bbox, float & xref, float & yref);
    ...
    public:
        ...
        */
        Follower3D(\
            ...
            ClassifierNames classifierName = GOOGLENET_TRT,\
            bool useCuda = false,
        ...
        */
         ...
         */
        TimedPose follow(bool est_pose = false, double est_x_h = 0, double est_y_h = 0);
